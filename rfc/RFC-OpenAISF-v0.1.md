...
- **Original** — 25 of 54 controls have no incumbent equivalent.
...
1. **11 control domains, 54 controls** (see `openaisf-controls.yaml`),
...
4. **A reference CLI** (`openaisf`) that evaluates a repo at a tier and
   emits a badge + JSON report. Prototype exists and runs.
5. **Sector Profile extensions** (healthcare, finance, public-sector,
  frontier) so the core stays lean (~54) while coverage is exhaustive.
...
- **D9-LTE — LLM Telemetry & Live Evaluation.** The canonical
...
```
$ openaisf init           # scaffold openaisf.yaml + POLICY.md + dirs
$ openaisf check --tier T1
✓ OpenAISF-T1: CONFORMANT (3 controls passed)
$ echo $?                 # 0
```
...
- The **reference CLI** is **Apache-2.0**.
...
1. **Domain structure.** Are 11 domains / 54 controls the right grain?
...
3. **D4-MPE (substitution safety).** Is the carrier-eval approach the
...
- It **emits** OSAA-SAFE-compatible incident reports (D9-04).
